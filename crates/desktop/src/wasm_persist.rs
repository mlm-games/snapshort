...
pub async fn init() -> Result<(), String> {
    ropfs::ensure_dir(keys::DIR_PROJECTS)
        .await
...
pub async fn save_autosave(data: &[u8]) -> Result<(), String> {
    ropfs::write(keys::FILE_AUTOSAVE, data)
        .await
...
pub async fn load_autosave() -> Option<Vec<u8>> {
    ropfs::read(keys::FILE_AUTOSAVE).await.ok()
}

/// Recent project names, most recent first.
pub async fn load_recent() -> Vec<String> {
    ropfs::read(keys::FILE_RECENT)
        .await
        .ok()
        .and_then(|b: Vec<u8>| serde_json::from_slice::<Vec<String>>(&b).ok())
        .unwrap_or_default()
}
...
async fn remember_recent(name: &str) {
    ...
    if let Ok(bytes) = serde_json::to_vec(&recent) {
        let _ = ropfs::write(keys::FILE_RECENT, &bytes).await;
    }
}
