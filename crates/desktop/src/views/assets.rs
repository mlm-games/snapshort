...
pub fn assets_panel(store: Rc<Store>) -> View {
    ...
    let list_row = Row(Modifier::new().flex_grow(1.0)).child(if assets.is_empty() {
        Column(
            Modifier::new()
                .fill_max_size()
                .align_items(repose_core::AlignItems::Center)
    ...
    let footer_row = Row(Modifier::new()
        .fill_max_width()
        .height(52.0)
        .background(colors::BG_PANEL)
        .border(1.0, colors::BORDER, 0.0)
    ...
    Column(Modifier::new().fill_max_size().background(colors::BG_DARK))
        .child((header_row, list_row, footer_row))
}
...
fn asset_item(asset: &snapshort_domain::Asset, idx: usize, store: Rc<Store>) -> View {
    ...
    let actions = Row(Modifier::new().align_items(repose_core::AlignItems::Center)).child((
